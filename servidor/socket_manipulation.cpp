...
// Inicializa o servidor para escutar na porta especificada
bool s_setup_server(uint32 port)
{
    ...
    if (res != 0)
    {
        _log("Getaddrinfo falhou! Código: {}", res);
        // s_shutdown_server();
        return false;
    }
    ...
    if (res == SOCKET_ERROR)
    {
        _log("Bind falhou com o código: {}", WSAGetLastError());
        freeaddrinfo(result);
        // s_shutdown_server();
        return false;
    }
    ...
    {
        _log("Função listen falhou com o código: {}", WSAGetLastError());
        // s_shutdown_server();
        return false;
    }
    u_long x = 1;
    ...
    if (res != 0)
    {
        _log("Erro ao tornar o socket assíncrono. Código: {}", WSAGetLastError());
        // s_shutdown_server();
        return false;
    }
    Sstarted = true;
...
// Disconecta todos e desliga o servidor
void s_shutdown_server()
{
    _log("Parando servidor...");
    if (!server_is_running())
    {
        _log("Erro... O servidor já está offline!!!");
        return;
    }
    ...
    mainsock = 0;
    // Caso tenha alguem desconectado, notifique a desconexao e termine de enviar os dados pendentes.
...
// Faz toda a comunicacao com o cliente, sendo enviando/recebendo dados ou aceitando novas conexoes
void s_sock_loop()
{
    static std::mutex mtx;
    std::unique_lock<std::mutex> lc(mtx);
    // Verificar se o servidor estï¿½ ativo e ouvindo...
    ...
    // Declarar extruturas de selecao e etc...
    FD_SET reads;
    FD_SET writes;
    FD_ZERO(&reads);
    FD_ZERO(&writes);
    FD_SET(mainsock, &reads);
    // Adicionar todos sockets para as verificacoes...
    if (connections.size() > 0)
    {
        for (auto it = connections.begin(); it != connections.end(); ++it)
        {
    ...
    {
        // Tentar aceitar a nova conexï¿½o...
        SOCKET s = accept(mainsock, NULL, NULL);
        if (s == SOCKET_ERROR)
        {
            _log("Erro ao aceitar conexões... {}", WSAGetLastError());
        }
        else
        {
            ...
            // Envia o socket por evento para que seja atribuido um objeto de conexao&player.
            dlb_event_send(event_connect, s);
        }
    }

    // Itera pelas conexoes ativas e verifica se pode receber ou enviar dados
    if (connections.size() > 0)
    {
        for (auto it = connections.begin(); it != connections.end(); ++it)
        {
            // Verificar se alguem enviou dados para nos...
            if (FD_ISSET(it->first, &reads))
            {
                ...
                if (res == SOCKET_ERROR)
                {
                    ...
                    // Envia um evento de desconexï¿½o...
                    dlb_event_send(event_disconnect, it->first);
                    continue;
                }
                else if (res == 0) // A conexao foi fechada...
                {
                    it->second->setSock(0);
                    it->second->setConState(con_disconnected);
                    dlb_event_send(event_disconnect, it->first);
            ...
            {
                // Envia dados, se possivel...
                std::string line = it->second->get_line_to_send();
                // Se nao tiver nada para enviar
                if (line.size() == 0)
                {
                    continue;
                }
...
// Adiciona uma nova conexao a lista de conexoes
