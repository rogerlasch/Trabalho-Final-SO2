...
// Estado que a mesa se encontra no momento
enum GAME_STATE
{
    g_starting = 0,
    g_playing,
...
// Diz que a mesa tem um atraso para que nao ocorra flood de mensagens (bots)
enum table_flags
{
    table_delay = (1 << 0)
};
...
Table::Table()
{
    this->setId(table_generate_id()); // Gera um id para a mesa
    current_turn.store(0); // Inicia o turno como 0 (primeiro jogador)
    gstate.store(g_starting); // Inicia o estado da mesa como "iniciando"
    pindex.store(0); // Inicia o indice do jogador como 0
    turn_dir.store(turn_right); // Inicia a direcao do jogo como "direita"
    active_players.store(0); // Inicia o numero de jogadores ativos como 0
    players.clear(); // Limpa a lista de jogadores
    deck.clear(); // Limpa o baralho
    discard.clear(); // Limpa a pilha de descarte
    acumulator.clear(); // Limpa o acumulador
    current_player = NULL; // Inicia o jogador atual como NULL
    current_card = NULL; // Inicia a carta atual como NULL
    winers.clear(); // Limpa a lista de vencedores

    // Inicia o mapa de comandos
    cmdtable = {
...
// Da um delay na mesa para dizer a cada quanto tempo e possivel mandar mensagem aos clientes conectados (bots)
...
// Verifica se a mesa esta com atraso, se sim, nao e possivel mandar mensagem para o cliente (bots)
...
// Faz o bot jogar se o jogador atual for um bot
...
// Diz ID da mesa, players e status, usado no comando para ver as mesas disponiveis
...
// Procura um player na mesa pelo socket/id
...
// Procura um player da mesa pelo nome
...
// Verifica se a mesa tem um administrador, caso nao tenha, tenta atribuir ao primeiro jogador que nao seja um bot
shared_player Table::has_admin()
{
    ...
    if (ch != NULL)
    {
        ...
        _echo(players, 0, "Agora {} � o administrador da mesa.", ch->getName());
    }
...
// Processa os comandos que o player ou bot enviou
void Table::process_command(shared_player &ch, const std::string &cmdline)
{
    ...
    {
        ch->print("Você esta na mesa errada, man!");
        return;
    }
...
/* Calcula quantas cartas o jogador tera que comprar caso nao tenha nenhuma carta valida para ser jogada
e acumular mais */
...
// Seleciona o proximo jogador que vai jogar, tomando como base o sentido da mesa e aplica o efeito da carta de pular vez
...
/* Depois de todas as validacoes, a carta e jogada, verificando se o jogo deve continuar ou nao
e se o poximo jogador deve comprar, caso tenha ocorrido alguma jogada com +2, +4, etc */
void Table::internal_PlayCard(shared_player &ch, shared_card &c, uint32 index)
{
    ...
    if (ch->deckSize() == 0)
    {
        ...
        _echo(players, 0, "{} agora � um expectador.", ch->getName());
        calculatePoints(ch);
...
// Invoca o metodo de selecao do proximo jogador e aplica as penalidades das cartas anteriormente jogadas (caso existam)
...
// Calcula os pontos que o jogador ira receber, somando as cartas dos oponentes
...
// Faz o bot jogar uma carta, pescar ou pular a vez
...
// Mostra os comandos disponiveis na mesa
...
// Vai pescar uma carta ou reembaralhar caso nao tenha mais
...
// O jogador/bot pode passar a vez caso tenha pescado pelo menos 1 carta
...
// Serve para gerenciar os bots na partida (adicionar ou remover)
void Table::cmd_bot(shared_player &ch, const std::string &args)
{
    ...
    if (this->get_gstate() == g_playing)
    {
        ch->print("O jogo já começou, o comando ficou indispon�vel.");
        return;
    }
...
// Mostra quem esta na mesa (inclui bots)
...
// Manda mensagens apenas para quem esta na mesa
...
// Define se voce sera um jogador ou espectador
void Table::do_quero(shared_player &ch, const std::string &args)
{
    ...
    if (args == "assistir")
    {
        ch->setflag(player_expectator);
        ch->removeflag(player_playing);
        ch->print("Agora voc� � um expectador.");
    }
    else if (args == "jogar")
    {
        ch->setflag(player_playing);
        ch->removeflag(player_expectator);
        ch->print("Agora voc� � um jogador.");
    }
    else
    {
        ch->print("Argumento inv�lido!");
        ch->print("Use \"quero ?\" para obter ajuda.");
    }
}

// Sinaliza que o jogador/bot disse "UNO"
void Table::do_uno(shared_player &ch, const std::string &args)
{
    ...
    if (this->get_gstate() != g_playing)
    {
        ch->print("O jogo ainda não come�ou!");
        return;
    }
...
// Mostra a classificacao dos jogadores pela pontuacao (inclui bots)
...
// Metodo para gerar um ID exclusivo para a mesa
uint32 table_generate_id()
{
    static uint32 x = 0;
    x++;
...
// Envia para uma lista de jogadores uma mensagem formatada
