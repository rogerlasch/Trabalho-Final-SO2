...
inline int32 random_int32(int32 min_val, int32 max_val);
//Preenxe um vector com a quantidade especificada de itens...
inline void fill_vector(std::vector<int32>& vect, uint32 size, int32 min_val=0, int32 max_val=100);
//Ler uma lista de números de um arquivo e preenxer o vetor...
inline bool fill_vector_from_file(const std::string& filename, std::vector<int32>& vect);
//Sobrecarga para printar um std::vector...
inline std::ostream& operator<<(std::ostream& os, const std::vector<int32>& vect);

inline void print_vector(const std::vector<int32>& vect,int32 n);
...
inline FuncTimer(const std::string& func_name);
inline ~FuncTimer();
};
#endif
